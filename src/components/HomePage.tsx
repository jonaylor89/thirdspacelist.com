...
export default function HomePage({ initialPlaces, initialLocation }: HomePageProps) {
  const { user } = useAuth()
  ...
  const [currentLocation, setCurrentLocation] = useState<[number, number]>(
    initialLocation || [-74.006, 40.7128] // Default to NYC
  )
  ...
  const fetchPlaces = useCallback(async () => {
    console.log('fetchPlaces', { currentLocation, filters });
    setLoading(true)
    try {
      ...
      if (useTypesense) {
        // Use new Typesense search API
        params.append('q', '*') // Search all
        ...
        params.append('sort_by', currentLocation ? 'location:asc' : 'workability_score:desc')

        const response = await fetch(`/api/search/places?${params}`)
        const data = await response.json()
        
        if (response.ok && data.places) {
          console.log(`Found ${data.found} places via Typesense search`)
          setPlaces(data.places)
        } else {
          console.error('Typesense search failed:', data.error)
        ...
        if (currentLocation) {
          const [lng, lat] = currentLocation as [number, number]
  ...
  // Fetch places when location or filters change (but not on initial mount since we have server data)
  useEffect(() => {
  ...
  }, [currentLocation, filters, initialLocation, fetchPlaces])
